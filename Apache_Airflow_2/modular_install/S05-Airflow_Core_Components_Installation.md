...
### Step 5.1: Install Airflow on Scheduler and Webserver Nodes (VM1, VM13, VM14)

**Execute on VM1 (airflow), VM13 (scheduler2), and VM14 (haproxy2):**
```bash
...
**Create HA-ready airflow.cfg on VM1:**
```bash
...
sql_alchemy_conn = postgresql://airflow_user:airflow_pass@192.168.83.210:5000/airflow_db
pool_size = 10
max_overflow = 20
pool_pre_ping = True
pool_recycle = 3600
...
[celery]
# RabbitMQ cluster configuration for HA
...
base_url = http://192.168.83.210:8081
...
**Distribute configuration from VM1 to other nodes:**
```bash
# Copy configuration to VM13 (scheduler2)
scp ~/airflow/airflow.cfg rocky@192.168.83.151:/home/rocky/airflow/
scp -r ~/airflow/config ~/airflow/utils rocky@192.168.83.151:/home/rocky/airflow/

# Copy configuration to VM14 (haproxy2) 
scp ~/airflow/airflow.cfg rocky@192.168.83.154:/home/rocky/airflow/
scp -r ~/airflow/config ~/airflow/utils rocky@192.168.83.154:/home/rocky/airflow/

# Copy SSH keys for task execution
for host in scheduler2 haproxy2; do
...
**Initialize database via VIP (run only once from VM1):**
```bash
...
### Step 5.5: Install Airflow on Worker Node (VM4)
...
### Step 5.6: Install Airflow DAG Processors (VM2, VM12)

**On VM2 (ftp) and VM12 (nfs2):**
```bash
# Install Python dependencies
sudo dnf install -y python3 python3-pip python3-devel gcc gcc-c++ postgresql-devel
...
**On VM1 (airflow) - Create scheduler and webserver services:**
```bash
...
**On VM13 (scheduler2) - Create scheduler service:**
```bash
...
**On VM14 (haproxy2) - Create webserver service:**
```bash
...
**On VM4 (worker1) - Create worker service:**
```bash
...
**On VM2 (ftp) and VM12 (nfs2) - Create DAG processor services:**
```bash
...
```bash
# On the active NFS server (check which one has VIP)
...
**1. Start DAG Processors first (VM2 - active NFS):**
```bash
# On the active NFS server (VM2 if it has VIP 192.168.83.220)
sudo systemctl start airflow-dag-processor
sudo systemctl status airflow-dag-processor
```

**2. Start Schedulers (VM1, VM13):**
```bash
# On VM1
...
# On VM13  
ssh rocky@192.168.83.151 "sudo systemctl start airflow-scheduler"

# Verify both schedulers are running
sudo systemctl status airflow-scheduler
ssh rocky@192.168.83.151 "sudo systemctl status airflow-scheduler"
```

**3. Start Webservers (VM1, VM14):**
```bash
# On VM1
...
# On VM14
ssh rocky@192.168.83.154 "sudo systemctl start airflow-webserver"

# Verify both webservers are running
sudo systemctl status airflow-webserver
ssh rocky@192.168.83.154 "sudo systemctl status airflow-webserver"
```

**4. Start Worker (VM4):**
```bash
# On VM4
ssh rocky@192.168.83.131 "sudo systemctl start airflow-worker"

# Verify worker is running
ssh rocky@192.168.83.131 "sudo systemctl status airflow-worker"
```
...
# VM13 services
echo "VM13 (scheduler2):"
ssh rocky@192.168.83.151 "sudo systemctl is-active airflow-scheduler" && echo "✅ Scheduler active" || echo "❌ Scheduler failed"

# VM14 services
echo "VM14 (haproxy2):"
ssh rocky@192.168.83.154 "sudo systemctl is-active airflow-webserver" && echo "✅ Webserver active" || echo "❌ Webserver failed"

# VM4 services
echo "VM4 (worker1):"
ssh rocky@192.168.83.131 "sudo systemctl is-active airflow-worker" && echo "✅ Worker active" || echo "❌ Worker failed"

# VM2 services (active NFS)
echo "VM2 (ftp - NFS active):"
...
echo "==============================================="
echo "✅ Scheduler HA: VM1 + VM13"
echo "✅ Webserver HA: VM1 + VM14 (load balanced)"
echo "✅ Worker: VM4"
echo "✅ DAG Processor HA: VM2 + VM12 (active/passive)"
echo "✅ Flower Monitor: VM1"
echo ""
echo "Access URLs:"
echo "- Airflow UI (Load Balanced): http://192.168.83.210:8081"
echo "- Flower: http://192.168.83.129:5555"
...
✅ **Multi-Scheduler HA**: VM1 + VM13 coordinat ed via database  
✅ **Load Balanced Webservers**: VM1 + VM14 via HAProxy  
✅ **Distributed Workers**: VM4 with horizontal scaling capability  
✅ **HA DAG Processing**: VM2 + VM12 coordinated with NFS failover  
